...
# Initialize environment and extract the MuJoCo universe
obs, _ = env.reset()
model_comp = env.unwrapped.model
data_comp = env.unwrapped.data

print("\n" + "="*50)
print(" STATE INJECTOR MODE")
print(" 1. The physics engine is strictly PAUSED.")
print(" 2. Commands:")
print("    'arm 0.5'   -> Teleport arm to 0.5")
print("    'pend 1.57' -> Teleport pendulum to 90 degrees")
print("    'reset'     -> Puts everything back to 0")
print("    [Enter]     -> Just prints the current observation")
print("                   (Useful if you drag the robot with your mouse!)")
print("="*50 + "\n")

with mujoco.viewer.launch_passive(model_comp, data_comp) as viewer:
    while viewer.is_running():
        # Wait for you to type something (blocks the terminal, but viewer stays alive!)
        user_input = input("\nEnter command (or press Enter to read): ").strip().lower().split()
        ...
        try:
            if len(user_input) > 0:
                ...
                if cmd == "reset":
                    ...
                    val = float(user_input[1])
                    # Ensure indices match your XML (0 = arm, 1 = pendulum)
                    if cmd == "arm":
                        data_comp.qpos[0] = val
                    elif cmd == "pend":
                        data_comp.qpos[1] = val
            
            # Update the kinematic tree (applies the positions without advancing time)
            mujoco.mj_forward(model_comp, data_comp)
            viewer.sync()
            
            # Get the exact observation the AI sees
            current_obs = env.unwrapped._obs()
            
            # Print it cleanly
          
            print(f"--> Sim Obs: {current_obs}")
            
        except Exception as e:
            print("Invalid input. Try 'arm 0.5', 'pend -0.5', or just press Enter.")
