...
# Hardware PWM safety frequency clamp (Jetson Tegra divisor lower limit)
...
DIR_PIN = 16
EN_PIN = 22
# Note: Hardware PWM step line is on physical Pin 32 (chip 0, channel 0)

# SPI for pendulum encoder
...
# Instantiate PWM controller (Pin 32 = pwmchip0, channel 0)
...
def cpu_control_loop():
    ...
    print(f"[CPU Thread] Motor zeroed. Starting {TARGET_HZ} Hz control loop.")

    # Initialize PWM output active at minimum floor frequency
    ...
    with torch.no_grad():
        ...
        while not stop_event.is_set():
            ...
            # 5. Position & Step Delta Calculation
            ...
            if actual_steps != 0:
                ...
                # Standstill: drop duty cycle to 0 so no pulses fire while keeping PWM active
                pwm_step.pause()

            # 7. Strict Timing Governor
            next_tick += PERIOD
...
if __name__ == "__main__":
    ...
    GPIO.setup(EN_PIN, GPIO.OUT, initial=GPIO.LOW)  # Active LOW driver enable
